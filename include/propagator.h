...
typedef struct {
  ...
  // size of zenith bins for the map (in km);
  double dzenith_map;
  int nzenith_map;

  // max zenith for the map
  double max_zenith_map;
  double min_zenith_map;

  // size of elev_elt bins for the map (in km);
  double delev_elt_map;
  int nelev_elt_map;

  // max elev_elt for the map
  double max_elev_elt_map;
  double min_elev_elt_map;

  // size of azim_elt bins for the map (in km);
  double dazim_elt_map; 
  int nazim_elt_map;

  // max azim_elt for the map
  double max_azim_elt_map;
  double min_azim_elt_map;

  // size of elev_surf bins for the map (in km);
  double delev_surf_map; 
  int nelev_surf_map;
  
  // max elev_surf for the map
  double max_elev_surf_map; 
  double min_elev_surf_map;

  // size of azim_surf bins for the map (in km);
  double dazim_surf_map;
  int nazim_surf_map;
  
  // max azim_surf for the map
  double max_azim_surf_map;
...
typedef struct {
  ...
  double      flattening;
  double      radius;
  // Earth body-fixed rotating frame for conversion ECI to ECEF  
...
typedef struct {
  ...
  // Their should not be more than 700 files in the GITM directory
  // you're looking at.

  char array_gitm_file[700][70]; 

  // column 0 is the time in Julian date, second column is the index
  // in array_gitm_file of the file which date is this one.
  double array_gitm_date[700][2];
  
  // represents the time (in Julian date) of the most recent GITM file
  int gitm_index_lower;

  // represents the time (in Julian date) of the subsequent GITM file  
  int gitm_index_higher;

  // matters if using GITM for the computation of the density is drag is
  // used. 1 if it is the first step of the run, 0 otherwise
  int is_first_step_of_run;

  // density of the GITM file which date is right before the epoch of the sc
  double ***density_gitm_right_before; 

  // density of the GITM file which date is right after the epoch of the sc
  double ***density_gitm_right_after;

  // lon/alt/lat of the GITM files
  double ***longitude_gitm, ***latitude_gitm, ***altitude_gitm; 

  // number of lat/lon/alt/variables in the GITM file
  int nLons_gitm, nLats_gitm, nAlts_gitm, nVars_gitm; 

  // index in altitude_gitm of the altitude that is right below the
  // perigee altitude (calculated at the initialization). Note: if the
  // duration of the run is long (so that the sc looses a good amount
  // of altitude, so like 6 months) then this index is set to 0 so
  // that we go over all the altitudes and not only the ones that
  // start below the perigee calcualted at the initialization.
  int index_altitude_right_below_perigee; 
...
typedef struct {
  ...
  // longitude of the ascending node OR RAAN (see output file ->
  // called RAAN)? As used in the code, it's the RAAN (see in
  // propagator.c the function cart2kep with eq (2-82) in Vallado3)
  double long_an;
  ...
  // zenith angle (angle earth to sc, sc to sun) varies form 0 to
  // 360. 0/360 deg if Sun at zenith of satellite, 90 deg if at
  // horizon of satellite and 'behind’ sc deg, 180 deg if Sun at nadir
  // of sc, 270 deg if Sun at horizon of sc and 'in front' of sc
  double zenith;
...
int jday (int year, int mon, int day, int hr, int minute, double sec,
	  double *jd );
...
int load_params( PARAMS_T *PARAMS,
		 int iDebugLevel,
		 char earth_fixed_frame[100] ,
		 double use_ap_hist,
		 int iProc,
...
int gitm_density( double *density, double et,
		  double altitude, double latitude, double longitude,
		  PARAMS_T *PARAMS);

int test_print( char to_print[1]  );

//int read_gitm_bin(double small_array[5],
//		  double ***longitude_gitm,
//		  double ***latitude_gitm,
//		  double ***altitude_gitm,
//		  double ***density_gitm,
//		  char time_gitm[256],
//		  int nLons,
//		  int nLats,
//		  int nAlts,
//		  char name_gitm_file_to_read[256]);
...
int calculate_cd_opengl(double *cd,
			double acco_coeff, 
			double v_sc_mag, //in km/s
			double Ta, // atmospheric temp in K, from NRLSMSIS
...
int polynomial_interpo(double *val,
		       int  n,
		       double *x,
		       double *y,
		       double to_inter);
...
int compute_iradius_gravity_map(int iradius_arr[2],
				GRAVITY_T *Gravity,
				double rmag);
...
int compute_ilat_gravity_map(int ilat_arr[4],
			     GRAVITY_T *Gravity,
			     double lat_gc);
...
int compute_ilon_gravity_map(int ilon_arr[4],
			     GRAVITY_T *Gravity,
			     double lon_gc_corr);
...
int gravity_map_yinter_lat(double *yinter,
			   int *order_interpo_map,
			   double lat_gc,
			   GRAVITY_T *Gravity,
			   double y_lat0,
			   double y_lat1,
...
int gravity_map_yinter_radius(double *yinter,
			      int *order_interpo_map,
			      double rmag,
			      GRAVITY_T *Gravity,
			      double y_radius0,
...
int gravity_map_xinter(double *xinter_lon,
		       double *xinter_lat,
		       double *xinter_radius,
		       double long_gc_corr,
		       double lat_gc,
		       double rmag,
		       GRAVITY_T *Gravity,
...
int gravity_map_yinter_lon(double *yinter,
			   int *order_interpo_map,
			   double long_gc_corr,
			   GRAVITY_T *Gravity,
			   double y_lon0,
...
int compute_iradius_earth_pressure_map(double *xinter_radius,
				       int iradius_arr[2],
				       int *nradius_interpo,
				       GRAVITY_T *Gravity,
				       double rmag);
...
int compute_izenith_earth_pressure_map(double *xinter_zenith,
				       int izenith_arr[2],
				       int *nzenith_interpo,
				       GRAVITY_T *Gravity,
				       double zenith);
...
int compute_ielev_surf_earth_pressure_map(double *xinter_elev_surf,
					  int ielev_surf_arr[2],
					  int *nelev_interpo,
					  GRAVITY_T *Gravity,
					  double elev_surf);
...
int compute_iazim_surf_earth_pressure_map(double *xinter_azim_surf,
					  int iazim_surf_arr[2],
					  int *nazim_interpo,
					  GRAVITY_T *Gravity,
...
int earth_pressure_map_yinter_azim_surf(double **yinter,
					int *order_interpo_map,
					double azim_surf_corr,
					GRAVITY_T *Gravity,
					double **y_az,
...
int earth_pressure_map_yinter_elev_surf(double **yinter,
					int *order_interpo_map,
					double elev_surf,
					GRAVITY_T *Gravity,
					double **y_el);
...
int earth_pressure_map_yinter_zenith(double **yinter,
				     int *order_interpo_map,
				     double zenith,
				     GRAVITY_T *Gravity,
				     double **y_zen);
...
int gravity_map_yinter_radius_earth(double **yinter,
				    int *order_interpo_map,
				    double rmag,
				    GRAVITY_T *Gravity,
				    double **y_rad);
...
int polynomial_interpo_earth(double *val,
			     int n,
			     double *x,
			     double **y,
			     double to_inter);
